...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ variant = 'primary', icon, children, className = '', ...props }, ref) => {
    const baseClasses =
      'inline-flex items-center justify-center rounded-md font-medium transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 dark:ring-offset-background disabled:opacity-50 disabled:pointer-events-none';
    const sizeClasses = variant === 'icon' ? 'h-10 w-10' : 'h-10 px-4 py-2';
    
    return (
      <motion.button
        ref={ref}
        className={`${baseClasses} ${sizeClasses} ${getVariantClasses(variant)} ${className}`}
        whileTap={{ scale: 0.97 }}
        {...props}
      >
